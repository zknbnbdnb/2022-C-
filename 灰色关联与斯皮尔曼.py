...
for i in range(len(data2[0])):
    ...
    for j in range(len(data2[0])):
        tmp2 = data2[:,j]
        tmp.append(spearmanr(tmp1, tmp2)[0])
        tmp_p.append(spearmanr(tmp1, tmp2)[1])
    res.append(tmp)
    res_p.append(tmp_p)
res = np.asarray(res)
...
colormap = plt.cm.RdBu
plt.figure(figsize=(18,16))
plt.title('spearmanr',y=1.05,size=18)

draw = sns.heatmap(res.astype(float),linewidths=0.1,vmax=1.0,square=True,\
            cmap=colormap,linecolor='white',annot=True)
plt.show()
draw.get_figure().savefig('spearmanr.png')
